...
namespace HandBrakeWPF.Themes
{
    ...
    public class ThemeLoader
    {
        ...
        public static void LoadAppTheme(IUserSettingService userSettingService)
        {
            ...
            // Clear existing theme/style dictionaries to allow dynamic switching
            var mergedDicts = Application.Current.Resources.MergedDictionaries;
            for (int i = mergedDicts.Count - 1; i >= 0; i--)
            {
                var src = mergedDicts[i].Source?.OriginalString;
                if (src != null && (src.Contains("Themes/") || src.Contains("Styles/")))
                {
                    mergedDicts.RemoveAt(i);
                }
            }

            mergedDicts.Add(new ResourceDictionary { Source = new Uri("Themes/Generic.xaml", UriKind.Relative) });
            mergedDicts.Add(new ResourceDictionary { Source = new Uri("Themes/BpressResources.xaml", UriKind.Relative) });
            bool themed = false;
