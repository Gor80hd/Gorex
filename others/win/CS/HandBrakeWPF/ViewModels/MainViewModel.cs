...
namespace HandBrakeWPF.ViewModels
{
    ...
    public class MainViewModel : ViewModelBase, IMainViewModel
    {
            ...
            : base(userSettingService)
        {
            ...
            // Ensure UI reflects current theme setting
            this.NotifyOfPropertyChange(() => this.IsDarkMode);
        ...
        public ICommand ToggleThemeCommand => new SimpleRelayCommand<object>(o => this.ToggleTheme());

        public ICommand CloseCommand => new SimpleRelayCommand<object>(o => Application.Current.MainWindow?.Close());

        public ICommand MinimizeCommand => new SimpleRelayCommand<object>(o =>
        {
            if (Application.Current.MainWindow != null)
                Application.Current.MainWindow.WindowState = WindowState.Minimized;
        });

        public ICommand MaximizeCommand => new SimpleRelayCommand<object>(o =>
        {
            if (Application.Current.MainWindow != null)
            {
        ...
        public bool IsDarkMode
        {
            get
            {
                var mode = (AppThemeMode)this.userSettingService.GetUserSetting<int>(UserSettingConstants.DarkThemeMode);
                // Default to Dark if not set to Light explicitly, matching the Bpress aesthetic
                return mode != AppThemeMode.Light;
            }
            set
            {
                this.userSettingService.SetUserSetting(UserSettingConstants.DarkThemeMode, (int)(value ? AppThemeMode.Dark : AppThemeMode.Light));
                ThemeLoader.LoadAppTheme(this.userSettingService);
                this.NotifyOfPropertyChange(() => this.IsDarkMode);
            }
        }
        ...
        private void ScanCompleted(object sender, ScanCompletedEventArgs e)
        {
            ...
            ThreadHelper.OnUIThread(() =>
            {
                if (e.Successful && this.ScannedSource != null)
                {
                    this.NotifyOfPropertyChange(() => this.ScannedSource);
                    this.NotifyOfPropertyChange(() => this.ScannedSource.Titles);
                    this.ShowSourceSelection = false; // Enable navigation first
                    this.HasSource = true; // Then flag source available
                    this.SelectedTitle = this.ScannedSource.Titles.FirstOrDefault(t => t.MainTitle) ?? this.ScannedSource.Titles.FirstOrDefault();
                }
                else if (!e.Cancelled)
                {
